...
export function checkArrayOfEntities2(entities: Array<any>, checkNonZeroLength: boolean, expectUniqueDatasetIds: boolean): Array<string> {
    ...
    for (const entity of entities) {
        ...
        result = result.concat(checkEntity2(entity, expectUniqueDatasetIds))
    }
...
export function checkReifiedAttribute(attribute: any, key: string, expectedType: string | undefined, expectUniqueDatasetIds: boolean): Array<string> {
    ...
    for (const instance of attribute) {
        ...
        if (instance["https://uri.etsi.org/ngsi-ld/observedAt"] != undefined && !isDateTimeUtcString(instance["https://uri.etsi.org/ngsi-ld/observedAt"][0])) {
            result.push(key + ": 'observedAt' is not a valid ISO 8601 UTC DateTime string.")
        }

        if (instance['https://uri.etsi.org/ngsi-ld/datasetId'] != undefined && !isUri(instance['https://uri.etsi.org/ngsi-ld/datasetId'][0]["@id"])) {
...
export function checkEntity2(entity: any, expectUniqueDatasetIds: boolean): Array<string> {
